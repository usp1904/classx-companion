...
function enforceLaTeX(text) {
  ...
  const issues = [];
  if (plaintextFraction.test(stripped)) issues.push('Found plaintext fraction like 1/2. Use $\\frac{1}{2}$');
  if (caretPower.test(stripped)) issues.push('Found caret power like x^2. Use $x^2$');
  if (plainOperators.test(stripped)) issues.push('Found plaintext operator pattern. Use LaTeX for expressions');
